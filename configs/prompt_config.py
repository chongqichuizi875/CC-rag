...
PROMPT_TEMPLATES = {
    ...
    "knowledge_base_chat": {
        "default":
            """
        <指令>根据已知信息，简洁和专业的来回答问题，答案必须尽可能简明扼要，字数不能超过30个字符。如果无法从中得到答案，请说 “根据已知信息无法回答该问题”，不允许在答案中添加编造成分，答案请使用中文。 </指令>
        <已知信息>{{ context }}</已知信息>
        <问题>{{ question }}</问题>
        """,
        "abstract":
            """
        <指令>你的任务是根据下面的已知信息生成一个简短摘要，摘要信息的长度不得超过256个字符，答案请使用中文。 </指令>
        <已知信息>{{ context }}</已知信息>
        """,
         "intention":
            """
        <指令>你是军事装备维修领域专家，用于解决军事装备，车辆等维修、保障等工作，请根据用户问题判断用户问题是闲聊还是咨询专业问题，如果是专业问题请返回是，否则返回否。 </指令>
        <示例>
            问题:你叫什么名字？
            回答:否
